...
from evaluate_NPR import evaluate_model
...
def get_model(num_users, num_items, mf_dim=10, layers=[10], reg_layers=[0, 0, 0, 0], reg_mf=0):
    ...
    MLP_embedding_user = Embedding (input_dim = num_users, output_dim = int(layers [0]),
                                    embeddings_initializer = 'random_normal',
                                    name = 'mlp_user_embedding', embeddings_regularizer = l2 (reg_mf), input_length = 1)
    MLP_embedding_item = Embedding (input_dim = num_items, output_dim = int(layers [0]),
                                    embeddings_initializer = 'random_normal',
    ...
    prefer_pos = merge.multiply([mf_user_latent, mf_item_latent_pos])
    prefer_neg = merge.multiply([mf_user_latent, mf_item_latent_neg])
    prefer_neg = Lambda (lambda x: -x) (prefer_neg)
    mf_vector = merge.concatenate([prefer_pos, prefer_neg])
    ...
    mlp_vector = merge.concatenate([mlp_user_latent, mlp_item_latent_pos, mlp_item_latent_neg])

    for idx in range (1, num_layer):
        mlp_vector = Dense (layers [idx], kernel_regularizer = l2 (0.0000),
                            activation = 'tanh', name = "layer%d" % idx) (mlp_vector)
...
if __name__ == '__main__':
    ...
    for epoch in range (num_epochs):
        ...
        if epoch % verbose == 0:
            ...
            print (
                'Iteration %d [%.1f s]: [Valid HR = %.4f, NDCG = %.4f, loss=%.6f]\t[Test HR = %.4f, NDCG = %.4f], [%.1f s]'
